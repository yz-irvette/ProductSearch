...
tf.flags.DEFINE_float("learning_rate_decay_rate", .10, "Learning Rate Decay Factor")
...
print("Loading data...NO.3")
train_df = pd.read_csv(os.path.join(FLAGS.data_dir, "rnn_train.csv"))
test_df = pd.read_csv(os.path.join(FLAGS.data_dir, "rnn_test.csv"))
validation_df = pd.read_csv(os.path.join(FLAGS.data_dir, "rnn_valid.csv"))
...
train_df.Utterance = pd.Series(train_df.Utterance, dtype=str)
...
def evaluate_rnn_predictor(df):
    ...
    for n in [1, 2, 5, 10]:
        f = open("log_1.txt","a+")
        print("Recall @ ({}, 10): {:g}".format(n, evaluate_recall(y, y_test, n)), file=f)
        f.close()

...
#define FEATURE_TO_PRODUCT_RATIO 0.5
...
map<string, vector<string> > feature_edge;
vector<string> nodes;
...
string RandNextNode(string from, float random) {
	...
	if (feature_edge.find(from) == feature_edge.end() || random < FEATURE_TO_PRODUCT_RATIO) {
		vector<Edge>& next = node_edge[from];
		random = random / FEATURE_TO_PRODUCT_RATIO
		...
		//return from;
		cout << " " << from;
		random = (random - FEATURE_TO_PRODUCT_RATIO) / (1 - FEATURE_TO_PRODUCT_RATIO)
				* (feature_edge[from].size() + 1);
		if (random >= feature_edge[from].size()) {
			return from;
		}
		return feature_edge[from][(int) random];
	}
}
...
/*
	...
	*/
	...
	while (!feof(fp)) {
		...
		float weight;
		fscanf(fp, "\"%[^\"]\",\"%[^\"]\"\n", in_a, in_b);
		feature_edge[string(in_a)].push_back(string(in_b));
	}
	fclose(fp);
